...
ggfun_graphs <- function() {
  p <-
    ...
    ggplot2::scale_x_continuous(
      limits = c(-1.5, 1.5),
      labels = c("Discrete", "Continuous", "Mixed"),
      breaks = c(-1, 0, 1),
      expand = c(0, 0)
    ) +
    ...
    ggplot2::annotate(
      ...
      x = 1,
      y = -.5,
      label = paste("Box and whisker plot", "(geom_boxplot)", sep = "\n"),
      size = 2.9
    ) +
    ggplot2::geom_rect(
      ggplot2::aes(xmin = .5, xmax = 1.5, ymin = 0, ymax = 1),
      fill = "gray"
    ) +
    ...
    ggplot2::theme(
      ...
      plot.caption = ggplot2::element_text(face = "bold", size = 8)
    )
  p
}
